...
function CheckIfLinksfromHanichlol() {
    ...
    $("#mw-content-text a").each(function () {
        ...
        var pCheck = pCheckKV[title] || new Promise(resolve => chrome.extension.sendRequest({ func: "ifHaveHamichlol", title: title , random: random }, resolve));
        pCheckKV[title] = pCheck;
        pCheck.then(response => {
            if (!response.have) {
                var $elm = $("<span>")
                    .text("M")
                    .addClass("ex-michlol-link");

                listNotHave[title] = href;
                $this.after($elm)
            }
        })
        pCheckList.push(pCheck);
    });
    ...
    Promise.all(pCheckList).then(()=>{
        ...
        if(nohaveList && nohaveList.length){
            /*
        <div class="toc">
            <div class="toctitle">
                <h2>תוכן עניינים</h2>
            ...
            */
            ...
            var $div = $('<div>').addClass('toc');

            $("<h2>").text("לא קיים במכלול").appendTo(
                $('<div>').addClass('toctitle').appendTo($div)
            );

            var $ol = $("<ol>");
            $ol.appendTo($div);
            nohaveList.forEach(i => {
                var li = $("<li>");
                var a = $("<a>");
                a.text(i).attr('href',listNotHave[i]).attr('target','_blank').appendTo(li)
                li.appendTo($ol);
            });
...
function selectPages(){
    return new Promise(function(resolve, reject) {
        ...
        $textarea.addClass("ex-textarea-select-page").appendTo($div);

        $textarea.val(Object.keys(listNotHave).join('\n'));
...
function selectClass(item){
    ...
    return new Promise(function(resolve, reject) {
        ...
        function addItem(name,text){
            var $item = $("<span>");
            $item.addClass("ex-item-class").text(name).appendTo($list);
        ...
        listClassification.forEach(function(mclass){
            mclass.list.forEach(function(sub){
                addItem(mclass.name + " | " + sub, "{{" + ["דף לטיפול","סיווג=" + mclass.name , "סיווג משנה=" + sub].join("|") + "}}");
            });
            $("<br>").appendTo($list);
        });

        addItem("בעיה כללית","{{דף לטיפול}}");
    ...
    chrome.extension.sendRequest({ func: "ifHaveHamichlol", title: pageTitle , random: random }, function (response) {
        ...
        if (response.have) {
            ...
            listButton.forEach(item => {
                ...
                var elmAdd = [$bodyContent];

                if(item.addHead)
                    elmAdd.push($("#firstHeading"));
